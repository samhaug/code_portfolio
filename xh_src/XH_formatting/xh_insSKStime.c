...
// Inserts SKS-wave traveltime into the XH header
...
while (still_reading_data) {
  ...
  if (! distaz(h.slat,h.slon,h.elat,h.elon,&delta,&az,&baz))
    exit (-1);
  // Get SKSwave traveltime for depth and distance from TravelTimeTables
  if (! gettime(h.edep,delta,&SKStime,"SKS"))
    exit (-1);
  // Determine StartTime-OriginTime
  if (! xh_TsT0_diff(h,&td) == 1)
    exit (-1);
  // Put SKStime in XH-header
  h.tpcks[8] = SKStime-td;
  fprintf(stdout,"to.yr= %d\n", h.ot.yr);
  fprintf(stdout,"ts.yr= %d\n", h.tstart.yr);
  ...
  fprintf(stdout,"%3d %-4s %-2s %-3s    Gcarc=%8.3f    SKS-time= %8.2f\n",
          icount,h.stnm,h.netw,h.chan,delta,h.tpcks[8]);
  icount++;
...
char *flnm;
        {
        fprintf(stderr,"Usage: xh_insSKStme in_XHfile out_XHfile\n");
        return(-1);
}
