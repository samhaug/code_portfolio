...
// Cuts a traces from origin time t0 to t0+7200.
...
int i_twohour;        // two hour worth of samples (dt)
int i0;
...
char *flnm;
        {
        fprintf(stderr,"Usage: xh_twohour in_XHfile out_XHfile\n");
        fprintf(stderr,"Generates 1-hour long traces starting at the earthuake origin time\n");
        return(-1);
}
