...
// Inserts ScS-wave traveltime into the XH header
...
while (still_reading_data) {
  ...
  if (! distaz(h.slat,h.slon,h.elat,h.elon,&delta,&az,&baz))
    exit (-1);
  // Get ScSwave traveltime for depth and distance from TravelTimeTables
  if (! gettime(h.edep,delta,&ScStime,"ScS"))
    exit (-1);
  // Determine StartTime-OriginTime
  if (! xh_TsT0_diff(h,&td) == 1)
    exit (-1);
  // Put ScStime in XH-header
  h.tpcks[7] = ScStime-td;
  ...
  fprintf(stdout,"%3d %-4s %-2s %-3s    Gcarc=%8.3f    ScS-time= %8.2f\n",
          icount,h.stnm,h.netw,h.chan,delta,ScStime);
  icount++;
...
char *flnm;
        {
        fprintf(stderr,"Usage: xh_insScStme in_XHfile out_XHfile\n");
        return(-1);
}
