...
/* ----------------------------------
 Additions and modification from 0.98
 ------------------------------------
 Sam takes over 0.98 -> 1.00

 Adding field for source/time function
 Adding for first arriving waveforms
 STF_NPTS is good for 50 s at 50 Hz, should cover all source/time funcitons
 and first arriving wave pulses
*/

#define X_VERSION	1.00
...
#define XH_NPTS		262144
#define STF_NPTS	2500
...
typedef struct {
    ...
    float   stf[STF_NPTS];		/* source time function	*/
    float   ttp[STF_NPTS];		/* First arriving P waveform	*/
    float   tts[STF_NPTS];		/* First arriving S waveform	*/
