...
int distaz(slat, slon, elat, elon, delta, az, baz)

   float slat, slon, elat, elon;
   float *delta, *az, *baz;

{
         double scolat, stalat, stalon, ecolat, evtlat, evtlon;
...
if ( fabsf(stalat-90.) < 0.01) stalat = stalat - 0.01;
if ( fabsf(stalat+90.) < 0.01) stalat = stalat + 0.01;
if ( fabsf(evtlat-90.) < 0.01) evtlat = evtlat - 0.01;
if ( fabsf(evtlat+90.) < 0.01) evtlat = evtlat + 0.01;
