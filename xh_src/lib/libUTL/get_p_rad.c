...
int get_p_rad(h,azim,inc,P_amp)
cmthed h;
float azim, inc, *P_amp;
{
    float P[3], SV[3];
...
//  fprintf(stdout,"%d %d %d\n", h.strike2, h.dip2, h.rake2);
   ...
   *P_amp   = Mxx*P[0]*P[0]+Myy*P[1]*P[1]+Mzz*P[2]*P[2]; 
   *P_amp  += Mxy*(P[0]*P[1]+P[1]*P[0]);
   *P_amp  += Mxz*(P[0]*P[2]+P[2]*P[0]);
   *P_amp  += Myz*(P[2]*P[1]+P[1]*P[2]);
