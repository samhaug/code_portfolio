...
float  Amp;
  ...
  if ( (Amp/L_noise) < SignalToNoise) {
   if (jlog) {
     fprintf(log,"%s [%s] discarded because S_amp/L_noise= %5.1f\n",
                  stnam,chnam,Amp/L_noise);
   }
   return;
  }
  /*  OUTPUT .... */
  if (jrvw) fprintf(rvw,"SL  %s\n", datcode);
  if (jrvw) write_review1(rvw,Ssample,Amp,L_noise,L_noise_window,envAmpRatio);
