...
/* ----------------------------------------------------
   Checks for TRACE2 where the maximum value is.
   If the maximum vaklue in TRACE1 is more than 35 s
   from its center [= Samp +/- i35s] then iTraceMax = 1 
  
   This ensures that we do not fit signal that is
   more than 35 s from the predicted arrival time
*/

max = 0.;
for (i=Samp-i40s; i<=Samp+i40s; ++i) {
...
if (! detrend(t1,npts_crc))
  fprintf(stderr,"Error in detrend -- processing() ..t1\n");
if (! detrend(t2,npts_crc))
  fprintf(stderr,"Error in detrend -- processing() ..t2\n");
...
if (Wvshape < 0. || Wvshape > 1.) Wvshape = 0.;
if (Amp1 > 100.) Amp1 = -99.99;
if (Amp2 > 100.) Amp2 = -99.99;
