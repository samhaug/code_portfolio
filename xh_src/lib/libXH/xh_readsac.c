...
fprintf(stdout,"a1 delta = %f\n", sh.delta);
if (sh.depmax == 0.) *useful = 0;
if (sh.depmin == 0.) *useful = 0;
fread(&seism[0],sh.npts*sizeof(float),1,sacf);

/* ABSOLUTE BEGIN TIME */
fprintf(stdout,"nzyear = %d\n", (int) sh.nzyear);
fprintf(stdout,"nzhour = %d\n", (int) sh.nzhour);
fprintf(stdout,"beg = %f\n", sh.b);
...
fprintf(stdout,"h.tstart.hr = %d\n", sh.nzhour);
fprintf(stdout,"h.tstart.mn = %d\n", sh.nzmin);
fprintf(stdout,"h.tstart.sec = %d\n", sh.nzsec);
